...
const Sidebar = () => {
  ...
  return (
   <section className="font-bai w-[18%] xl:block hidden h-screen bg-gray-900 pb-[20px] overflow-y-auto no-scrollbar z-[10] sticky top-0">
    {/* ----------------------sidebar------------------------- */}
    ...
    <h3 className="text-blue-400 text-[16px] ">{providers?.length}</h3>
    </div>
 {/* Provider List */}
 <ul className="custom-scrollbar mt-2 space-y-2 bg-gray-900 rounded-lg">
          {providers.map((provider, index) => (
            <li
              key={index}
              className="flex items-center space-x-2 p-2 bg-gray-800 rounded-md hover:bg-gray-700 cursor-pointer"
            >
              <img
        ...
        {/* No Providers Found */}
        {providers.length === 0 && (
          <p className="text-gray-400 text-center mt-4">No providers available.</p>
        )}
    </div>
